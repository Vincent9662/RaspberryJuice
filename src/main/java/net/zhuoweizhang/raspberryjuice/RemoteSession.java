...
public class RemoteSession {
	...
	protected void handleCommand(String c, String[] args) {
		...
		try {
			...
			if (c.equals("world.getBlock")) {
			...
			} else if (c.equals("world.strikeLightning")) {
				try {
					Location loc = parseRelativeBlockLocation(args[0], args[1], args[2]);
					world.strikeLightning(loc);
					// send("");
				} catch (Exception e) {
					plugin.getLogger().warning("Error striking lightning: " + e.getMessage());
					send("Error");
				}

				// Lightning effect without damage
			} else if (c.equals("world.strikeLightningEffect")) {
				try {
					Location loc = parseRelativeBlockLocation(args[0], args[1], args[2]);
					world.strikeLightningEffect(loc);
					// send("");
				} catch (Exception e) {
					plugin.getLogger().warning("Error striking lightning effect: " + e.getMessage());
					send("Error");
				}
				...
				// player.getTileLookingAt
			} else if (c.equals("player.getTileLookingAt")) {
				Player currentPlayer = getCurrentPlayer();
				int distance = 200;
				if (args.length > 0) {
				...
				Block block = currentPlayer.getTargetBlock((java.util.Set<org.bukkit.Material>) null, distance);
				if (block != null) {
				...
				// Player-based lightning commands
			} else if (c.equals("player.strikeLightning")) {
				try {
					Player currentPlayer = getCurrentPlayer();
					if (currentPlayer != null) {
						...
						Location targetLoc = playerLoc.clone().add(x, y, z);
					...
					plugin.getLogger().warning("Error in player.strikeLightning: " + e.getMessage());
					send("Error");
				}
			} else if (c.equals("player.strikeLightningEffect")) {
				try {
					Player currentPlayer = getCurrentPlayer();
					if (currentPlayer != null) {
						Location playerLoc = currentPlayer.getLocation();
					...
					plugin.getLogger().warning("Error in player.strikeLightningEffect: " + e.getMessage());
					send("Error");
				}
			} else if (c.equals("player.strikeLightningAtSelf")) {
				try {
					Player currentPlayer = getCurrentPlayer();
					if (currentPlayer != null) {
						world.strikeLightning(currentPlayer.getLocation());
						// send("");
					} else {
						send("No player");
					}
				...
				try {
					int entityId = Integer.parseInt(args[0]);
					// IMPORTANT: Use plugin.getEntity(), not getEntity()
					...
					plugin.getLogger().warning("Error in entity.strikeLightning: " + e.getMessage());
					send("Error");
				}
			} else if (c.equals("entity.strikeLightningEffect")) {
				try {
					int entityId = Integer.parseInt(args[0]);
					Entity entity = plugin.getEntity(entityId);
				...
				try {
					entityType = EntityType.fromId(Integer.parseInt(args[3]));
				} catch (Exception e) {
